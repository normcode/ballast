defmodule Ballast.ClientTest do
  ...
  describe "Ballast.Client" do
    ...
    test "sets client timeout with option", _ctx = %{bypass: origin,
      ...
      error = assert_raise Tesla.Error, fn -> Client.request(client, request) end
      assert error.reason == :timeout
      assert_receive :slow_reply, 1000
    end

    test "sets connect timeout with option", _ctx = %{bypass: bypass,
                                                      client: client} do
      Bypass.down(bypass)
      request = [method: :get,
                 url: "http://10.255.255.1/unroutable",
                 opts: [connect_timeout: 1] ] # in ms
      error = assert_raise Tesla.Error, fn -> Client.request(client, request) end
